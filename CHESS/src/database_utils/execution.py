...
def execute_sql(db_path: str, sql: str, fetch: Union[str, int] = "all", timeout: int = 60) -> Any:
    """
    Executes an SQL query on a SQLite database, supporting various fetch modes.

    Args:
        db_path (str): Path to the SQLite database file or directory containing it.
        sql (str): The SQL query to execute.
        fetch (Union[str, int]): Fetch mode ('all', 'one', 'random', or an integer).
    ...
    class QueryThread(threading.Thread):
        ...
        def run(self):
            try:
                ...
                # 如果 db_path 是目录：
                # 搜索目录下的所有 .sqlite 文件
                if db_path_resolved.is_dir():
                    sqlite_files = list(db_path_resolved.glob("*.sqlite"))
                    if not sqlite_files:
                        raise FileNotFoundError(f"No SQLite file found in directory: {db_path}")
                    db_path_resolved = sqlite_files[0]  # Use the first .sqlite file found
                # 尝试连接 SQLite 数据库并执行传入的 SQL 查询
                with sqlite3.connect(str(db_path_resolved), timeout=timeout) as conn:
                    cursor = conn.cursor()
                    cursor.execute(sql)
                    if fetch == "all":
    ...
    query_thread.start()
    # 使用 join(timeout) 等待线程在指定的 timeout 时间内完成
...
def _clean_sql(sql: str) -> str:
    ...
    Also decodes HTML entities like &lt; to <.
