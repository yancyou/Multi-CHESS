...
class DatabaseManager:
    ...
    def _init(self, db_mode: str, db_id: str):
        ...
        # 局部敏感哈希
        self.lsh = None
        # 于快速估计集合相似度的 MinHash 技术
        self.minhashes = None
        # 向量数据库，用于存储和查询向量化的数据
        self.vector_db = None

    # 加载或初始化 vector_db 属性，用于处理与上下文向量数据库
    def _set_paths(self):
        """Sets the paths for the current database."""
        self.db_directory_path = self.db_root_path / self.db_id
        self.db_path = self.db_directory_path / f"{self.db_id}.sqlite"

    def set_lsh(self) -> str:
        """Sets the LSH and minhashes attributes by loading from pickle files."""
        try:
            preprocessed_path = self.db_directory_path / "preprocessed"
            start_time = time.time()
            with (preprocessed_path / f"{self.db_id}_lsh.pkl").open("rb") as file:
                self.lsh = pickle.load(file)
            after_lsh_time = time.time()
            with (preprocessed_path / f"{self.db_id}_minhashes.pkl").open("rb") as file:
                self.minhashes = pickle.load(file)
            print(f"LSH loaded for {self.db_id} in {after_lsh_time - start_time:.2f} seconds.")
            return "success"
    ...
    def set_vector_db(self) -> str:
        ...
        try:
            ...
            print(f"Vector DB loaded for {self.db_id}.")
            return "success"
        except Exception as e:
